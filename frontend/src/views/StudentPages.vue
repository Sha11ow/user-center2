                ...
                <el-row>
                  ...
                  <el-col :span="8">
                    <el-form-item label="上课时间" prop="Time">
                      <el-input v-model="queryInfo.Time" placeholder="请输入上课时间"></el-input>
                    </el-form-item>
                  </el-col>
                </el-row>
...
export default {
  ...
  methods: {
    ...
    async queryCourses() {
      ...
      try {
        ...
        if (courseData) {
          ...
          const courseNamePromises = courseData.map(async (course) => {
            const course_name = await getCourseName(course.course_id);
            return { ...course, course_name };
          });

          const teacherNamePromises = courseData.map(async (course) => {
            const teacher_name = await getTeacherName(course.teacher_id);
    ...
    async getCourseName(courseId) {
      // 替换为实际的API URL
      const courseNameApiUrl = `${this.host}/user/api`;
    ...
    async getTeacherName(teacherId) {
      // 替换为实际的API URL
      const teacherNameApiUrl = `${this.host}/user/api`;
      const response = await axios.get(teacherNameApiUrl, { params: { teacher_id: teacherId } });
      return response.data.teacher_name;
    },


    // 更新选课功能中的选择的课程到selectedCourse
